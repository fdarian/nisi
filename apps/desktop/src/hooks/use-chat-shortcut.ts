 ...
 * unclaimed by both the frontend and the macOS menu accelerators registered
 * in `src-tauri/src/lib.rs` (only ⌘W/⌘⇧W), so the webview always sees the
 ...
 */
export function useChatShortcut(onToggle: () => void): void {
	useEffect(() => {
