...
- bd47343: You can now choose the syntax theme the diff is rendered with, picking separately for light and dark mode.
...
- 3d1aae6: Support GitHub stacked pull requests. PRs in a stack now show their position in the header badge (e.g. "2/2") with a layers icon; clicking opens a popover listing the stack. Merging a PR with unmerged PRs below it merges the whole stack atomically.
...
- 6ac68c6: Navigate PR views with ⌘[ and ⌘] back/forward shortcuts within each tab.
- 6e07899: Long code blocks and URLs in chat replies now wrap inside the panel instead of running off the edge.
- 53cbc7f: Text in the chat panel is now selectable.
...
- 3f12248: Cycle full-file tabs with ⌘⇧[ and ⌘⇧]. When a full-file view is active, these shortcuts step through its open files; the chat popup still takes priority if it's open.
...
- aeeee28: Add markdown file preview support in the file view with GitHub-flavored markdown syntax and proper styling.
...
- 483e91e: PR tabs now close immediately instead of waiting for agent teardown.
