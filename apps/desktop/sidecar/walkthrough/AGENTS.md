...
`walkthrough.get` / `walkthrough.generate` procedures. Neither of those two packages does I/O or
knows about the other — this directory is where they actually meet.
...
- `harnesses.ts` — `listHarnesses` (the static+Pi-discovered registry `walkthrough.harnesses`
  returns) and `createHarnessAdapter` (harness/model choice → a real `HarnessV1` adapter instance).
...
- The three non-Pi harnesses' `STATIC_MODELS` in `harnesses.ts` are best-effort and unverified
  against the real CLIs — a stale id fails at `generate` time, not at `harnesses` time (there's no
  `isAvailable` API to check against up front). Revisit as the underlying CLIs change.
